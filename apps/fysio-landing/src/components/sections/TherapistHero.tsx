...
export function TherapistHero({ therapist, bookingUrl }: TherapistHeroProps) {
            ...
            {therapist.specialties && therapist.specialties.length > 0 && (
              <div className="mt-6 flex flex-wrap gap-2 justify-center lg:justify-start">
                {therapist.specialties.map((specialty, index) => (
                  <span
                    key={index}
                    className="inline-flex rounded-full border border-primary/20 bg-primary/8 px-4 py-2 text-sm font-medium text-primary"
                  >
                    {specialty}
                  </span>
                ))}
